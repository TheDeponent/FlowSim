...
def generate_HalfExtension(
    ...
    # Handle moves in a circle of radius arm_length / 2
    poi_handle_x = poi_length * np.cos(theta_1)
    poi_handle_y = poi_length * np.sin(theta_1)
    # Head moves in a circle of radius (arm_length / 2) + poi_length
    poi_head_x = (arm_length / 2 + poi_length) * np.cos(theta_1)
    poi_head_y = (arm_length / 2 + poi_length) * np.sin(theta_1)
